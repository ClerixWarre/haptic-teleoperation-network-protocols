...
// Stream configuration for surgical applications
...
struct SendContext {
    std::vector<uint8_t> buffer;  // Directly own buffer
...
// Initialize SCTP
bool InitializeSCTP(const char* serverName) {
    if (g_sctp_initialized) {
        return true;  // Already initialized
    }

    logInfo("Initializing SCTP...");
    
    // Create socket
    g_socket_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_SCTP);
    if (g_socket_fd < 0) {
        logError("Failed to create SCTP socket: " + std::string(strerror(errno)));
        return false;
    }
    
    // Set events to be notified for
    struct sctp_event_subscribe events;
    memset(&events, 0, sizeof(events));
    events.sctp_data_io_event = 1;  // Enable data IO events
    events.sctp_association_event = 1;
    events.sctp_send_failure_event = 1;
    
    if (setsockopt(g_socket_fd, IPPROTO_SCTP, SCTP_EVENTS, &events, sizeof(events)) < 0) {
    ...
    if (fcntl(g_socket_fd, F_SETFL, flags | O_NONBLOCK) < 0) {
        logError("Failed to set socket to non-blocking mode: " + std::string(strerror(errno)));
        close(g_socket_fd);
        g_socket_fd = -1;
        return false;
    }
    ...
    if (setsockopt(g_socket_fd, IPPROTO_SCTP, SCTP_INITMSG, &initmsg, sizeof(initmsg)) < 0) {
    ...
    // Set no delay option (equivalent to TCP_NODELAY)
    int nodelay = 1;
    if (setsockopt(g_socket_fd, IPPROTO_SCTP, SCTP_NODELAY, &nodelay, sizeof(nodelay)) < 0) {
        logError("Failed to set SCTP_NODELAY: " + std::string(strerror(errno)));
        close(g_socket_fd);
        g_socket_fd = -1;
        return false;
    }
    ...
    g_server_addr.sin_port = htons(ServerPort);
    
    // Resolve hostname to IP
    struct hostent *host = gethostbyname(serverName);
    if (!host) {
        logError("Failed to resolve hostname: " + std::string(serverName));
        close(g_socket_fd);
        g_socket_fd = -1;
        return false;
    }
    
    memcpy(&g_server_addr.sin_addr, host->h_addr, host->h_length);
    
    logInfo("SCTP initialized successfully");
    g_sctp_initialized = true;
    return true;
}
...
bool ConnectToServer() {
    if (!g_sctp_initialized) {
        logError("SCTP not initialized");
        return false;
    }
    
    logInfo("Connecting to SCTP server...");
    ...
    if (getsockopt(g_socket_fd, SOL_SOCKET, SO_ERROR, &error, &len) < 0 || error != 0) {
        logError("Connection failed: " + std::string(strerror(error ? error : errno)));
        return false;
    }
    
    // Connection successful
    logInfo("Connected to SCTP server");
    
    // Mark all streams as ready
    for (int i = 0; i < NUM_STREAMS; i++) {
...
bool InitializeROS() {
    ...
    try {
        ...
        // Create node
        g_node = std::make_shared<rclcpp::Node>("sctp_haptic_client");
...
// Thread to receive SCTP responses
void StartReceiveThread() {
    std::thread([&]() {
        logInfo("Starting SCTP receiver thread");
        ...
        while (!shutdownRequested && g_socket_fd >= 0) {
            // Reset buffer for new data
            memset(buffer.data(), 0, buffer.size());
            
            // Receive with stream information
            socklen_t len = sizeof(sndrcvinfo);
            flags = 0;
            
            int n = sctp_recvmsg(g_socket_fd, buffer.data(), buffer.size(),
                                 NULL, NULL, &sndrcvinfo, &flags);
                                 
            if (n > 0) {
                // Successfully received data
                if (g_logConfig.enableStreamLogging) {
                    logDebug("Received " + std::to_string(n) + " bytes on stream " + 
                             std::to_string(sndrcvinfo.sinfo_stream));
                }
                
                // Process the response
                try {
                    if (n >= 16) {  // At least sequence + timestamp
                        uint64_t responseSeq = 0;
                        uint64_t responseTimestamp = 0;
                        
                        memcpy(&responseSeq, buffer.data(), sizeof(uint64_t));
                        memcpy(&responseTimestamp, buffer.data() + sizeof(uint64_t), sizeof(uint64_t));
                        ...
                        
                        {
                            ...
                            if (it != g_message_timings.end()) {
                                ...
                                g_metrics.messagesReceivedPerStream[sndrcvinfo.sinfo_stream]++;
                                g_metrics.totalBytesReceived += n;
                                g_metrics.addLatencySample(timing.rttMs, sndrcvinfo.sinfo_stream);
                            ...
                            if (!matchFound) {
                                ...
                                g_metrics.addLatencySample(estimatedRtt, sndrcvinfo.sinfo_stream);
                    ...
                    logError("Error processing response: " + std::string(e.what()));
                }
            }
            else if (n < 0) {
                ...
                // If connection is broken, exit the loop
        ...
        logInfo("SCTP receiver thread stopped");
    }).detach();
}

// Start real-time data thread to send haptic data
void StartRealTimeDataThread() {
    ...
    std::thread([&]() {
        ...
        while (!shutdownRequested.load() && !g_sendState.shutdownInitiated.load()) {
            ...
            if (g_socket_fd < 0 || connectionClosed.load()) {
                logWarning("Connection lost, stopping message thread");
                break;
            }
            ...
            if (hasEmergencyMessage) {
                ...
                // Try to find a ready stream - use stream 0 for emergency
                int streamIndex = 0;
                
                // Set up SCTP specific info
                struct sctp_sndrcvinfo sinfo;
                memset(&sinfo, 0, sizeof(sinfo));
                sinfo.sinfo_stream = streamIndex;
                sinfo.sinfo_flags = SCTP_UNORDERED;  // Send unordered for emergency
                sinfo.sinfo_ppid = htonl(EMERGENCY); // Use priority as payload protocol identifier
                ...
                // Send using sctp_sendmsg with priority info
                int flags = 0;
                int sent = sctp_sendmsg(g_socket_fd, 
                                       emergencyMessage.data.data(), 
                                       emergencyMessage.data.size(),
                                       (struct sockaddr*)&g_server_addr,
                                       sizeof(g_server_addr),
                                       htonl(EMERGENCY),  // ppid
                                       flags,             // flags
                                       streamIndex,       // stream number
                                       0,                 // timetolive
                                       0);                // context
                
                if (sent < 0) {
                    logError("Failed to send emergency message on stream " + 
                            std::to_string(streamIndex) + ": " + std::string(strerror(errno)));
                ...
                // Use stream 1 for high priority
                int streamIndex = 1;
                ...
                memset(&sinfo, 0, sizeof(sinfo));
                sinfo.sinfo_stream = streamIndex;
                sinfo.sinfo_flags = 0;  // Use ordered delivery
                sinfo.sinfo_ppid = htonl(HIGH); // Use priority as payload protocol identifier
                ...
                // Send using sctp_sendmsg
                int flags = 0;
                int sent = sctp_sendmsg(g_socket_fd, 
                                       highPriorityMessage.data.data(), 
                                       highPriorityMessage.data.size(),
                                       (struct sockaddr*)&g_server_addr,
                                       sizeof(g_server_addr),
                                       htonl(HIGH),     // ppid
                                       flags,           // flags
                ...
                static int roundRobinIndex = 2;  // Start with stream 2 for normal messages
                int streamIndex = roundRobinIndex;
                roundRobinIndex = (roundRobinIndex + 1) % NUM_STREAMS;
                if (roundRobinIndex < 2) roundRobinIndex = 2;  // Keep in range 2-3
                ...
                sinfo.sinfo_stream = streamIndex;
                sinfo.sinfo_flags = 0;  // Use ordered delivery
                sinfo.sinfo_ppid = htonl(NORMAL); // Use priority as payload protocol identifier
...
int main(int argc, char* argv[]) {
    ...
    logInfo("Starting SCTP Haptic Data Client...");
    ...
    try {
        ...
        // Initialize SCTP
        if (!InitializeSCTP(ServerName)) {
            logError("Failed to initialize SCTP");
            ShutdownROS();
            return 1;
        }
        ...
        logInfo("Connected to SCTP server, streaming haptic data...");
